# Binwalk Qemu Builder

## 简介

此目录包含Binwalk的Qemu集成构建脚本，专注于Windows平台上的Qemu功能集成。构建脚本在`builder`目录下创建隔离的构建环境，确保构建过程的一致性和可靠性。

## 特性

- **Qemu功能集成**：专注于Windows平台上的Qemu安装、Docker管理和GUI连接
- **隔离的构建环境**：所有依赖都安装在`builder/local_env`目录下
- **自动下载和配置**：自动下载并配置所需的构建工具和依赖
